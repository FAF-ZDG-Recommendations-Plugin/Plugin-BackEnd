    ...
    with open(sql_file_path, "rb") as file:  # Read in binary mode
        for line in file:
            ...
            for match in matches:
                ...
                post["embedding"] = model.encode(post["content"]).tolist()  # Generate embedding
...
for post in posts:
    ...
    # Check if the post is already indexed
    if post_already_indexed(post_id):
        print(f"Skipping already indexed post ID {post_id}")
        continue  # Skip already indexed posts
    ...
    res = client.index(
        index=INDEX_NAME,
        id=post_id,  # Set the document ID to match the post ID
        body=post,
        refresh=True
    )
    print(f"Indexed from: {post['post_date']} with URL: {post['guid']}")
